import { EMPTY_STRING, TYPE_FUNCTION, TYPE_STRING } from './const'
...
export class ParentNodeAssembler extends ChildNodeAssembler {
    ...
    find(subject, filter) {
        ...
        let selector = typeof subject === TYPE_STRING? subject : subject.selector
        ...
        if(node) {
            ...
            const elementAssembler = isAttr || typeof subject === TYPE_STRING?
                this.constructor.elementAssembler :
                subject
            const element = this.getInstanceOf(node, elementAssembler)
            return isAttr? element.getAttributeNode(subject) : element
        }
        return null
    }
    ...
    findAll(subject, filter) {
        ...
        let selector = isString? subject : subject.selector
        if(typeof filter === TYPE_STRING) {
            selector += filter
        }
        ...
        return typeof filter === TYPE_FUNCTION? results.filter(filter) : results
    }
     ...
     */
    prepend(...childNodes) {
        this.node.prepend(...flatten(childNodes).map(getNodeOf))
    }

    /**
     * @param {ChildNodeAssembler|Node|*} child
     * todo => ChildNodeAssembler.remove()
     */
    removeChild(child) {
     ...
     * @param {ChildNodeAssembler|Node|*} child
     * @returns {ChildNodeAssembler|*}
     * todo => ChildNodeAssembler.replaceWith()
     */
