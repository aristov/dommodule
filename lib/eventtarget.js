...
export class EventTargetAssembler extends Assembler {
    ...
    on(type, callback, options) {
        ...
        const handlers = this.__event_handlers__[Boolean(useCapture)]
        const map = handlers[type] || (handlers[type] = new Map)
        if(!map.has(callback)) {
            map.set(callback, callback = callback.bind(this))
    ...
    un(type, callback, options) {
        const useCapture = typeof options === 'boolean'?
            options :
            options && options.capture || false
        const map = this.__event_handlers__[Boolean(useCapture)][type]
        if(map) {
