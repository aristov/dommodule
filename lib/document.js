import { Document, DocumentType, Element, document } from './dom'
import { DEFAULT_NAMESPACE_URI } from './const'
...
export class DocumentAssembler extends ParentNodeAssembler {
    ...
    get doctype() {
        ...
        if(doctypeNode) {
            const doctype = this.getInstance(doctypeNode)
            if(doctype) return doctype
            else {
                const { doctypeAssembler } = this.constructor
                return new doctypeAssembler({ node : doctypeNode })
            }
        }
        else return null
    }
    ...
    set documentElement(documentElement) {
        ...
        if(documentElement instanceof ElementAssembler) {
            ...
            if(element) element.init(documentElement)
                ...
                const init = assign({ parentNode : node }, documentElement)
                new elementAssembler(init)
            }
        }
    }
    ...
    get documentElement() {
        ...
        if(elementNode) {
            const element = this.getInstance(elementNode)
            if(element) return element
            else {
                const { elementAssembler } = this.constructor
                return new elementAssembler({ node : elementNode })
            }
        }
        else return null
    }
