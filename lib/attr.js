...
export class AttrAssembler extends NodeAssembler {
    ...
    static resolveNodeType(node) {
        const assembler = super.resolveNodeType(node)
        if(ElementAssembler.isPrototypeOf(assembler)) {
            const namespaceURI = node.namespaceURI || EMPTY_STRING
            return assembler.namespace === namespaceURI && assembler.localName === node.localName?
                assembler :
