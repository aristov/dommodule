import { EMPTY_STRING, NAMESPACE_SEPARATOR } from './const'
...
export class AttrAssembler extends NodeAssembler {
    ...
    set parentElement(parentElement) {
        if(this.node.ownerElement) {
            ...
            throw Error(`Failed to set 'parentElement' on '${ this.constructor.name }': the 'ownerElement' is null.`)
        }
    }
    ...
    set parentNode(parentNode) {
        if(this.node.ownerElement) {
            ...
            throw Error(`Failed to set 'parentNode' on '${ this.constructor.name }': the 'ownerElement' is null.`)
        }
    }
    ...
    static init(selector = this.selector) {
        ...
        return map.call(document.querySelectorAll(selector), node => {
            return new this({ node : node.attributes[localName] })
        })
    }
