...
/**
 * @see {@link https://www.w3.org/TR/dom/#interface-element}
 */
export class ElementAssembler extends ParentNodeAssembler {
    ...
    appendChild(child) {
        ...
        if(child instanceof AttrAssembler) {
            ...
            const ownerElement = child.ownerElement
            if(ownerElement) {
                node.appendChild(ownerElement)
                return this.getInstanceOf(child)
            }
            else {
                throw Error(`Failed to append an Attr as a child on '${ this.constructor.name }': it has no owner element.`)
            }
        }
        else return super.appendChild(child)
    }
    ...
    getElementsByClassName(classNames) {
        ...
        const assembler = this.constructor.elementAssembler
        return map.call(nodes, node => this.getInstanceOf(node, assembler))
    }
    ...
    getElementsByTagName(object) {
        let nodes, assembler
        const node = this.node
        const isFunction = typeof object === TYPE_FUNCTION
        const isAssembler = object === ElementAssembler || ElementAssembler.isPrototypeOf(object)
    ...
    setProperty(name, value) {
        if(!['namespace', 'localName', 'qualifiedName', 'prefix'].includes(name)) {
    ...
    get children() {
        const assembler = this.constructor.elementAssembler
        return map.call(this.node.children, node => this.getInstanceOf(node, assembler))
    }
