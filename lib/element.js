...
export class ElementAssembler extends ParentNodeAssembler {
    ...
    getAttributeNode(attr) {
        ...
        let attrNode
        if(isFunctionType && attr.prototype instanceof AttrAssembler) {
            const { namespaceURI, qualifiedName, localName } = attr
        ...
        if(attrNode) {
            if(isFunctionType && attr.prototype instanceof AttrAssembler) {
                return new attr({ node : attrNode })
            }
            else return this.getInstance(attrNode)
        }
        else return null
    }
    ...
    removeAttributeNode(attr) {
        ...
        else attrNode = node.removeAttributeNode(attr)
        if(attrNode) {
            const instance = this.getInstance(attrNode)
            if(instance) return instance
            else {
                const { attrAssembler } = this.constructor
                return new attrAssembler({ node : attrNode })
            }
        }
        else return null
    }
    ...
    get attributes() {
        ...
        return map.call(this.node.attributes, node => {
            return this.getInstance(node) || new attrAssembler({ node })
        })
    }
    ...
    get children() {
        ...
        return map.call(this.node.children, node => {
            return this.getInstance(node) || new elementAssembler({ node })
        })
    }
