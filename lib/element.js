...
export class ElementAssembler extends ParentNodeAssembler {
    ...
    set id(id) {
        if(document.getElementById(id)) {
            throw Error(`Failed to set 'id' on '${ this.constructor.name }': the same id already exists in the document.`)
        }
        this.node.id = id
    }
    ...
    static register() {
        ...
        const separator = namespace && localName?
            SPACE :
            EMPTY_STRING
        this.registry[namespace + separator + localName] = this
    }
    ...
    static resolveElement(node) {
        ...
        const assembler = namespaceURI?
            resolver[namespaceURI + SPACE + localName] || resolver[namespaceURI] :
            resolver[localName]
