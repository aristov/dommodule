...
export class ElementAssembler extends ParentNodeAssembler {
    ...
    removeAttribute(attr) {
        const node = this.node
        if(attr.prototype) {
            node.removeAttribute(attr.qualifiedName)
        }
    ...
    removeAttributeNode(attr) {
        ...
        return this.getInstanceOf(this.node.removeAttributeNode(attr), isFunction && attr)
    }

    /**
     * this.setAttribute('attr', 'foobar')
     * this.setAttribute(AttrAssembler, 'foobar')
     * this.setAttribute(document.createAttribute('attr'))
     * this.setAttribute(new AttrAssembler({ value : 'foobar' }))
     * this.setAttribute(this.getAttr('attr'), 'foobar')
     * this.setAttribute(this.node.getAttribute('attr'), 'foobar')
     *
     * @param {class|string} attr
    ...
    setAttributeNode(attr) {
        ...
        let replacedNode
        if(attr instanceof AttrAssembler) {
            replacedNode = node.getAttributeNode(attr.name)
            attr.ownerElement = node
        }
        else replacedNode = node.setAttributeNode(attr)
