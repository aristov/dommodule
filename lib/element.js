...
export class ElementAssembler extends ParentNodeAssembler {
    ...
    /**
     * setAttributeNode(new Lang)
     * setAttributeNode(document.createAttribute('lang'))
     * 
    ...
    setAttributeNode(attr) {
        ...
        else throw TypeError('Failed: parameter is not of expected type.')
        if(replacedNode) {
     ...
     * getAttributeNode('lang')
     *
     * @param {Function|{namespaceURI,qualifiedName}|String} attr
     * @returns {AttrAssembler|null}
     */
    getAttributeNode(attr) {
        const isStringType = typeof attr === 'string'
        const namespaceURI = isStringType? '' : attr.namespaceURI
        const qualifiedName = isStringType? attr : attr.qualifiedName
        const node = this.node
        const attrNode = namespaceURI?
            node.getAttributeNode(qualifiedName) :
            node.getAttributeNodeNS(namespaceURI, qualifiedName)
        if(attrNode) {
            const instance = this.getInstance(attrNode)
            const assembler = isStringType || typeof attr !== 'function'?
