...
Object.defineProperty(NodeAssembler, 'AttrAssembler', { value : AttrAssembler })

/*
 * !!! Workaround !!!
 * The garbage collector in Safari removes instances of AttrAssembler
 *  that are not referenced from the static memory structure.
 */
const { userAgent } = window.navigator
if(/Safari/.test(userAgent) && !/Chrome/.test(userAgent)) {
    AttrAssembler.__instances__ = new Set
    Object.defineProperty(AttrAssembler, 'setTargetOf', {
        configurable : true,
        writable : true,
        value : function(instance, target) {
            NodeAssembler.setTargetOf(instance, target)
            AttrAssembler.__instances__.add(target)
        }
    })
}
