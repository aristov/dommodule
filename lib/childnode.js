...
export class ChildNodeAssembler extends NodeAssembler {
    ...
    after(...items) {
        ...
        this.node.after(...items.map(item => constructor.getNodeOf(item)))
    }
    ...
    before(...items) {
        const constructor = this.constructor
        this.node.before(...items.map(item => constructor.getNodeOf(item)))
    }
    ...
    replaceWith(...items) {
        const constructor = this.constructor
        this.node.replaceWith(...items.map(item => constructor.getNodeOf(item)))
    }
